...
function totalReport() {
	...
	$sql = "SELECT * FROM f32ee.jj_total_sales;";
	if (!$result = mysqli_query($conn, $sql)) {
		echo "Failed to get total sales: " . mysqli_error($conn);
	}
	for($i = 1; $i <= $result->num_rows; $i++) {
		$row = $result->fetch_assoc();
		$sql = "SELECT product_name FROM f32ee.jj_products WHERE product_id = $i;";
...
function quantReport() {
	...
	$sql = "SELECT sum(quantity) FROM f32ee.jj_orders GROUP BY product_id;";
	if (!$result = mysqli_query($conn, $sql)) {
